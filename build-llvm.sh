...
# only build the LLVM gold plugin `LLVMgold.so` for native Linux builds
# prevent build LLVM gold plugin when corss compile from Linux to windows
...
if [ -n "$WITH_ZSTD" ]; then
    ...
    # match libzstd.a for macOS and libzstd.dll.a for Windows
